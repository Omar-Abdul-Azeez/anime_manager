...
def main():
    parser = ArgumentParser(description='Anime library manager. Helps keep library in a uniform structure. folder template: "([anime-id] )?[anime title]", episode template: "[anime title] [episode-number].[ext]".', exit_on_error=False)
    subparsers = parser.add_subparsers()
    auth = subparsers.add_parser('auth', help='Auth mode, use this mode to get/set an authorization token.', exit_on_error=False)
    ...
    library = subparsers.add_parser('library', help='Library mode, covers and renames each anime folder inside a library directory and it\'s episodes to be consistent with Anilist.', exit_on_error=False)
    ...
    season = subparsers.add_parser('season', help='Season mode, creates anime folders for each currently-airing anime in the user\'s watching list on Anilist and covers them.', exit_on_error=False)
    ...
    rss = subparsers.add_parser('rss', help='RSS mode, runs watching mode in the specified qbittorrent category\'s save path then modifies qbittorrent\'s rss config to save each anime in it\'s folder directly with no need for guessing.', exit_on_error=False)
    ...
    episodes = subparsers.add_parser('episodes', help='Episodes mode, renames all video files in the root directory by guessing which anime in the user\'s watching list is it from.\nIf anime folders already exist by using rss or season modes, renames episodes inside those folders so that they follow "[folder title] [episode-number].[ext]".', exit_on_error=False)
    ...
    tsumi = subparsers.add_parser('tsumi', help='Tsumi mode (Requires authorization), adds yet-to-be-watched anime in library to Anilist customlist and removes completed anime from it. (completed anime needs to be in the library)', exit_on_error=False)
